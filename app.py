...
# Streamlit app
st.title("Image Stitching App (Panorama Stitching)")

# Upload two images
uploaded_images = st.file_uploader("Upload two images as input", type=["jpg", "jpeg", "png"], accept_multiple_files=True)

if uploaded_images is not None and len(uploaded_images) == 2:
    # Read and resize the uploaded images
    images = [Image.open(image) for image in uploaded_images]
    imageA = np.array(images[0])
    imageB = np.array(images[1])
